# MagiaMagica 技術選定・実装着手ガイド v0.1 (draft)

> 位置づけ: `magia-spec-v0.1.md` §3 のアーキテクチャ決定を、クレートレベルの具体選定と実装順序に落とす文書。INDEX v0.5 の「今後追加が想定される文書」のうち `magia-implementation-guide-v0.1.md` のたたき台に相当する。
> 作成日: 2026-06-10
...
├── crates/
│   ├── magia-core/    # IR定義・解析・レイアウト・SVGレンダラ
│   ├── magia-rust/    # Rust → IR アダプタ
│   └── magia-cli/     # CLI
└── fixtures/             # スナップショットテスト用サンプルコード
```

IR定義は将来 `magia-ir` として独立クレート化する可能性があるが、Phase 1 では `magia-core::ir` モジュールで開始し、Phase 2 着手時に分離を判断する（早すぎる分割を避ける）。
...
- **Phase 1b（必要になったら）**: `ra_ap_hir` / `ra_ap_ide` による意味解決を `magia-rust` 内のオプション機能（feature flag `semantic`）として追加。core の IR には影響しない
...
```
magia render <FILE> --fn <NAME> --layers control_flow,effects -o out.svg
...
- シリアライズ: `serde` + `serde_json`（IR の `--emit-ir` デバッグ出力。将来の `magia-ir-schema` 文書の実体化にもなる）
