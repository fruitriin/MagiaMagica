# MagiaMagica プロジェクト文書 INDEX v0.5

## プロジェクト名と用語整理（史実）

本プロジェクトの名称・関連用語は以下のとおり整理する。本文書群の本文では現行名 **MagiaMagica** を用いる。

- **MagiaMagica** — 本プロジェクトの現行名。
- **Mystical CI** — 本プロジェクトの **初期コードネーム**。設計セッション初期の文書群はこの名で書かれていた。現行文書では MagiaMagica に置換済み。旧版文書は `project-docs/magia/archive/` 配下に当時の本文のまま保存する（ディレクトリ自体は `mystical-ci/` → `magia/` に改名済み）。
- **Mystical lang** — 発想の起点となった魔法陣型プログラミング言語（Denis M. 作, 公式: https://suberic.net/~dmm/projects/mystical/ ）。GIGAZINE 記事「魔法陣のようなプログラミング言語『Mystical』」(2025-05-19) で言及される `Mystical` 本体を本プロジェクトでは便宜上 **Mystical lang** と呼ぶ。
  - MagiaMagica および前身の Mystical CI はいずれも Mystical lang の図形思想から影響を受けているが、**Mystical lang のコードは含まず・依存もしない**。MagiaMagica は「既存のコードを魔法陣として読む」逆方向のアプローチを取る。

---

## プロジェクト概要

**MagiaMagica** は、ソースコードを魔法陣状の図形に変換することで、コードレビューとリファクタリングにおける注意誘導を支援する、**三重の目的**を持つコード可視化・設計支援ツールである。
...
**`magia-spec-v0.1.md`** — MagiaMagica 仕様書 v0.1

Phase 1 のスコープを厳密に定義した正式仕様書。

**`spec-v0.2.md`** — MagiaMagica 仕様書 v0.2 ★追加 (2026-06-11)

v0.1 を基底とする第1増補。Phase 1 実装 (M1〜M7 + 1.8) の追認と Phase 2 の実装契約。

**`spec-v0.3.md`** — MagiaMagica 仕様書 v0.3 ★追加 (2026-06-11)

第2増補。Phase 3 (Spell Diff / CI 統合 / ベルカ式) の実装契約。
**実装時の主参照文書 (記載のない節は v0.2 → v0.1 の順で有効)**。

**`magia-phase2plus-notes-v0.1.md`** — Phase 2 以降の議論ノート v0.1

Phase 2 から Phase 6 までの構想を集約。長期ロードマップの主参照文書。

### 補遺文書 (現行バージョン)

**`magia-appendix-A-rendering-styles-v0.2.md`** — Appendix A: 描画様式カタログ v0.2

10式 (深層式 Profundus 含む) と二軸分類モデル。

**`magia-appendix-B-codebase-landscape-v0.1.md`** — Appendix B: コードベース景観とモジュール結合 v0.1

結合点 (Junction Glyph)、結合トポロジー、フラクタル景観、品質指標。

**`magia-appendix-C-theoretical-foundations-v0.2.md`** — Appendix C: 理論的基礎 v0.2

裁量 vs 規範の二項対立、ソフトウェア哲学史的位置づけ。

**`magia-appendix-D-design-mode-v0.2.md`** — Appendix D: 設計支援モードと式駆動設計 v0.2

Clean Architecture / POSD / Fusion レシピ、Advisor / Enforcer / Hybrid スタンス。

**`magia-appendix-E-design-philosophies-v0.2.md`** — Appendix E: 設計哲学カタログ v0.2 ★更新

**v0.2 でメタ分類軸を新規導入**。設計書を5タイプ (パターン集型 / 単一哲学型 / 戦略+戦術型 / 態度・原則型 / 規範書型) に分類。GoF を「カタログ式 (Catalog Style)」として位置づけ、新しい構造的概念を導入。

**`magia-appendix-F-scope-and-limits-v0.1.md`** — Appendix F: ツールの射程と限界 v0.1 ★新規
...
1. `magia-spec-v0.1.md` を熟読
2. `magia-phase2plus-notes-v0.1.md` で将来計画を確認
...
- `magia-implementation-guide-v0.1.md`
- `magia-ir-schema-v0.1.md`

### 中期 (Phase 2-3 実装期)

- `magia-filter-dsl-spec-v0.1.md`
- `magia-layout-algorithm-v0.1.md`
- `magia-glyph-symbol-catalog-v0.1.md`
- `magia-designer-mode-spec-v0.1.md`
- `magia-recipe-collection-v0.1.md`
- `magia-stance-configuration-guide-v0.1.md`

### 長期 (Phase 4 以降)

- `magia-appendix-G-language-adapters-v0.1.md`
- `magia-appendix-H-dynamic-analysis-v0.1.md`
- `magia-appendix-I-accessibility-v0.1.md`
- `magia-appendix-J-style-refactoring-v0.1.md`
- `magia-appendix-K-organizational-patterns-v0.1.md`
- `magia-appendix-L-catalog-style-rendering-v0.1.md` ★v0.5 新規 — カタログ式の表現様式専用文書
- `magia-appendix-M-ai-integration-v0.1.md` ★v0.5 新規 — AI 注釈チャネルの詳細
...
- `magia-user-guide.md`
- `magia-cli-reference.md`
- `magia-cookbook.md`
- `magia-design-tutorial.md`
- `magia-philosophy-reader.md`
- `magia-stance-tutorial.md`
- `magia-scope-faq.md` ★v0.5 新規 — 「これはツールでできますか?」によくある質問

---

## 命名規則 (v0.1 から変更なし)

`magia-<カテゴリ>-<内容>-<バージョン>.md`
