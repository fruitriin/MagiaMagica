...
| 11 | 4.0.6 | [docs/plans/phase4.0.6-circle-affordances.md](docs/plans/phase4.0.6-circle-affordances.md) | **完了**（前半 = 凡例 2026-06-11 / 後半 = 入口サイン + 補助陣ラベル 2026-06-13。判定待ち） |
...
| 17 | 4.5 | [docs/plans/phase4.5-workspace-overview.md](docs/plans/phase4.5-workspace-overview.md) | **M1 + M2 前段完了**（2026-06-12。俯瞰 + ?scope= + ファイル横断 call graph (workspace_index) + カード関係マーク。M2 本実装はオーナー方針待ち） |
...
| 18.7 | 4.10 | [docs/plans/phase4.10-promote-local-calls-to-aux-rings.md](docs/plans/phase4.10-promote-local-calls-to-aux-rings.md) | 未着手（**ローカル呼び出しを補助陣に昇格** — 同ファイル/ワークスペースで解決できる関数呼び出しを補助陣化して名前ラベルが見える。オーナー要望 2026-06-13） |
...
- **4.10 (ローカル呼び出しの補助陣昇格) はオーナー要望 (2026-06-13)**。4.5 M2 前段 (workspace_index) や 4.2 (function_index) の既存解決を再利用 — serve 層で IR 後加工する案 A 推奨。4.0.6 後半の補助陣ラベル機構をそのまま流用
