...
## 監視ファイルの切替 (Phase 4.4.5)

- watcher スレッドへの制御は **FS イベントと同じ channel に enum で統合**する
  (std mpsc に select が無い)。**デバウンスループが制御メッセージを飲んだら
  退避して後処理する** — パターンマッチで読み捨てると「切替したのに旧ファイル
  監視のまま」の silent drop になる (レビューで検出)
- 切替時のファイル名公開は **スナップショットの一括スワップに一本化**する。
  ファイル名と解析結果を別 Mutex に持つと「新しい一覧 + 旧ファイル名」の
