...
| [wasm-thin-wrapper-crate.md](wasm-thin-wrapper-crate.md) | 既存 Rust パイプラインをブラウザ単体で動かす WASM 薄ラッパー crate の定型 (ネイティブ依存の crate 隔離で wasm 化が無修正で通る, wasm-bindgen をターゲット限定依存に, pure 関数を native テスト + #[cfg] mod wasm に境界隔離, JSON in/out で HTTP 契約サブセット, wasm-bindgen CLI バージョン固定, serve 組み立てのコピー禁止=drift, Vite 別ビルドターゲットで本体に wasm 非混入, 動的 import + 注入で純粋化, 生成物不在でも通すワイルドカード ambient 宣言, erasableSyntaxOnly では関数ファクトリ) | wasm, wasm32-unknown-unknown, wasm-bindgen, cdylib, target.cfg dependencies, JsError, クライアント完結, magia-hobby, ブラウザ, JSON境界, wasm-opt, schema version mismatch, ネイティブ依存隔離, 薄ラッパー, (a)案, (b)案, drift, Vite, 別ビルドターゲット, dist-hobby, WasmDataSource, 動的import, ambient宣言, ワイルドカードモジュール, erasableSyntaxOnly, ?url, Pick サブセット型 |
