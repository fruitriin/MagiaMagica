...
## 鎖の途中のクロージャ陣は直交方向に伸ばす (Phase 4.8 M2 後 — 2026-06-13)

- 鎖の glyph (係留元が SummonGlyph) のクロージャ陣を「外向き = 鎖の進行方向」に
  伸ばすと、次の glyph と次のクロージャ陣の場所を奪い合って補助陣どうしが重なる
  (オーナー判定で発覚)。係留元が SummonGlyph のとき**進行方向と直交 (+π/2)** に
  伸ばすと自然に上下分散になり、鎖の流れを横切らない
- 「先頭の glyph (係留元がリング)」と「鎖の途中の glyph (係留元が glyph)」で
  基準角を切り替えるのが最小コスト — 兄弟扇形分散はそのまま重ねられる
