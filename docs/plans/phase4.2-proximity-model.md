...
- **Proximity トレイトは作らなかった**: 4.1 実装結果の確定方針どおり `classify_neighbors` 関数の中身を差し替えた (計画書のトレイト前提は 4.1 着手前の記述)。連続値モデル + 描画側離散化という本質はそのまま
- **「Phase 3.4 DataflowIr の call_edges」は実在しなかった**: dataflow IR は関数内の変数 def/use で、関数間エッジは持たない。呼び出し関係は magia-rust の **`function_index_with_calls`** (索引 + エッジを1パースで返す新 API) として実装 — index walker + summon の call site 収集 + 同ファイル解決 (qualified 完全一致 → 同 impl の名前一致 → 定義順先頭、の3段。`Self::` は impl 文脈で置換、マクロ・自己再帰・重複は除外)
- **リング境界は 1.1 → 0.9 に変更**: 同ファイル天井 (1.0) が全関数に効くため、計画書の境界 (≤1.1 中) では「呼び出し 0.7」と「同ファイル 1.0」が同じ中リングになり視覚差が出ない。0.6/0.9 にして「同 impl = 内 / 呼び出し = 中 / その他 = 外」の3層に
- **2ホップ係数 (1.4) は同ファイルスコープでは実質出ない** (min 合成で 1.0 に食われる)。ファイル外スコープ (4.5) で生きる係数として正しく実装し、テストで固定した
- UI 側マッピング表 (距離→リング) は Vue でなく **focus_layout (Rust の描画層)** に置いた — 「配置は Rust」の POSD 分担に整合。Vue は無修正 (distance: number 契約のまま 1/2/3 が来る)

### 副産物

- focus_layout の viewBox を「実際に使われた最遠リング」基準に修正 (4.1 レビュー S1 の引き継ぎ対応)
- serve は GoodSnapshot に call_edges をキャッシュ (reload 時1回構築、/spell 毎の再パースなし)

### レビュー (Stage 2) 対応

- W1: `function_index` → `call_graph` の2回パースを `function_index_with_calls` に統合 (1パース)
- W2: 同名メソッドの照合は呼び出し元と同じ impl を優先 (`self.render()` が他 impl の同名に誤エッジしない)。回帰テスト追加
- S2 (ring_value の clamp) / S3 (clone 1回) は実害なしで見送り

### 後続への引き継ぎ

- 4.4 (呼び出しジャンプ): call_edges は向き付きで取得済み — 近接度は無向で潰しているが、エッジ自体は (caller, callee) のまま serve にある
- 4.5 (ファイル外): call_hops の2段手展開をキュー式 BFS に書き直す (コメント済み)。距離係数に「モジュール階層」成分を足す
- 距離係数・リング境界の調整は `proximity.rs` の WEIGHT_* / `ir_export.rs` の RING_BOUNDARY_* に集約済み (オーナー判定で注文が出たらここだけ)
