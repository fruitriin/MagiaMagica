...
- 計画どおりに完了。**live diff (保存 → SSE → 金ハロー出現) は e2e で実 git により検証済み**
  (serve-fixture.sh を git init 化し「HEAD = 初期内容」を固定 — beforeEach の書き戻しと
  両立する形)
- viewBox の合成は「diff 版 ir をそのまま focus_layout の入力にする」だけで整合した
  (実装時に決めるとした論点 — diff_spell_ir が ir 自体を差し替える設計が効いた)
- レビュー対応: `-` 始まりの rev を弾く (git オプション注入防御)、before を
  キャッシュしない理由 (live diff の成立条件) をコメント化
- 素材: serve.rs の render_spell を ?diff=HEAD で表示 — 「diff 機能を実装した差分自身が
  live diff で見える」自己ホスティングの円環

### 4.6 に残るもの

テーマ切替 / ベルカ式の diff・同時表示 / before・after トグル / パーマリンク
