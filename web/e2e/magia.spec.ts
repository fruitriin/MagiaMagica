...
test("既定表示: 魔法陣・ソース・関数目次が出る (先頭関数へ fallback + URL 書き戻し)", async ({
  ...
  // ソースペインはファイル全体 (細部修正 2026-06-12)。フォーカス関数は範囲強調。
  await expect(page.locator(".src-line").first()).toContainText("fn greet");
  await expect(page.locator(".src-focus").first()).toContainText("fn greet");
});
...
test("関数目次クリックで ?pin= が切り替わり、戻るで復帰する", async ({ page }) => {
  ...
  await expect(page.locator(".src-focus").first()).toContainText("fn compute");
  // ファイル全体は表示されたまま (greet も見えている)。
  await expect(page.locator(".src-line").first()).toContainText("fn greet");
  await page.goBack();
  await expect(page).toHaveURL(/\?pin=greet/);
  await expect(page.locator("header span").first()).toHaveText("greet");
...
test("関数リスト: ピンを切り替えても並び順 (定義順) が変わらない (細部修正)", async ({ page }) => {
  await page.goto("/?pin=greet");
  const labels = async () =>
    (await page.locator("nav button").allInnerTexts()).map((t) => t.split("(")[0]?.trim());
  await expect(page.locator("nav button")).toHaveCount(3);
...
test("ソースペイン: Cmd+クリックで関数名から魔法陣を切り替える (細部修正)", async ({ page }) => {
  await page.goto("/?pin=greet");
  await expect(page.locator(".src-line").first()).toBeVisible();
  // helper の定義行の関数名を Cmd+クリック → ?pin=helper。
  const helperLine = page.locator(".src-line", { hasText: "fn helper" });
  await helperLine.getByText("helper", { exact: true }).click({ modifiers: ["ControlOrMeta"] });
  await expect(page).toHaveURL(/\?pin=helper/);
  await expect(page.locator("header span").first()).toHaveText("helper");
  await expect(page.locator(".src-focus").first()).toContainText("fn helper");
});

test("周辺チップの引数表示: パレットの変数名/型名チェックで切り替わり URL に残る (細部修正)", async ({
  page,
}) => {
  await page.goto("/?pin=compute");
  const helperChip = page.locator("svg g[role=button]", { hasText: "helper" });
  await expect(helperChip).toBeVisible();
  await expect(helperChip.locator("text.chip-args")).toHaveCount(0);
  // メイン魔法円のシグネチャは既定 (両方 OFF) では Rust の text そのまま。
  const mainSig = page.locator("svg text.signature").first();
  await expect(mainSig).toContainText("a : i32");
  await page.getByText("⚙ パレット").click();
  // 変数名のみ → チップ (value) / メイン円 fn compute(a, b) -> i32。
  await page.getByLabel("変数名").check();
  await expect(helperChip.locator("text.chip-args")).toHaveText("(value)");
  await expect(mainSig).toHaveText("fn compute(a, b) -> i32");
  await expect(page).toHaveURL(/args=name/);
  // 型名も → (value: i32) / fn compute(a: i32, b: i32) -> i32。
  await page.getByLabel("型名").check();
  await expect(helperChip.locator("text.chip-args")).toHaveText("(value: i32)");
  await expect(mainSig).toHaveText("fn compute(a: i32, b: i32) -> i32");
  await expect(page).toHaveURL(/args=name%2Ctype|args=name,type/);
  // URL 直開きで復元 (型名のみ)。
  await page.goto("/?pin=compute&args=type");
  await expect(helperChip.locator("text.chip-args")).toHaveText("(i32)");
  await expect(page.locator("svg text.signature").first()).toHaveText(
    "fn compute(i32, i32) -> i32",
  );
});

test("?file= の復元は1回だけ: 外部でファイルが切り替わったらサーバに追従する (細部修正)", async ({
  page,
}) => {
  // URL 直開きの復元 (1回目) は機能する。
  await page.goto("/?file=orbit.rs");
  await expect(page.locator("header span").first()).toHaveText("orbit", { timeout: 10_000 });
  // 外部 (別タブ相当) がサーバの監視ファイルを切り替える。
  await page.evaluate(() =>
    fetch("/file", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ path: "sample.rs" }),
    }),
  );
  // このタブは ?file=orbit.rs を主張し返さず、サーバへ追従して URL を書き換える
  // (複数タブの綱引き防止)。
  await expect(page.locator("header span").first()).toHaveText("greet", { timeout: 10_000 });
  await expect(page).toHaveURL(/file=sample\.rs/);
});

test("メソッドチェーン: 鎖の後続が数珠つなぎになり細線で連結される (Phase 4.8)", async ({
  page,
}) => {
  // orbit.rs の label.trim().chars().filter(|c| ..).count() が4連鎖 → Chain edge 3本。
  await page.goto("/?file=orbit.rs&pin=orbit");
  await expect(page.locator("header span").first()).toHaveText("orbit", { timeout: 10_000 });
  const chainLines = page.locator("svg line.edge-chain");
  await expect(chainLines).toHaveCount(3);
  // .filter のコールバックは補助陣 (Phase 4.8 M2): orbit は制御構造なしなので
  // 補助リング = クロージャの1個だけ。中に .is_alphabetic の glyph が入る。
  await expect(page.locator(".pin-view circle.aux-ring")).toHaveCount(1);
  // 鎖はリング係留線 (1.0) より細い (0.6)。
  await expect(chainLines.first()).toHaveAttribute("stroke-width", "0.6");
  // 鎖の glyph は一直線 (先頭から外向き放射): 2本の線分の傾きがほぼ一致する。
  const slopes = await chainLines.evaluateAll((lines) =>
    lines.map((l) => {
  ...
  expect(Math.abs(slopes[0]! - slopes[1]!)).toBeLessThan(0.01);
  // 戻し (他テストは sample.rs 前提)。
  await page.getByLabel("監視ファイル").selectOption("sample.rs");
  await expect(page.locator("header span").first()).toHaveText("greet", { timeout: 10_000 });
});
