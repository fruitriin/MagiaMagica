...
use magia_core::render::RenderStyle;
...
fn run_render(
    ...
    // Phase 4.3 M5: SVG 出力は全経路 Vue SSR — 動的 UI と同じコンポーネントで
    // 描く (Rust SVG レンダラは削除済み)。フィルタは適用結果に畳んで渡す。
    let svg = match style {
        RenderStyle::MidchildaConcentric => {
            let placed = layout(&graph);
            let ir = magia_core::render::ir_export::spell_ir(&graph, &placed);
            ssr::render_via_ssr(&ssr::render_request(&ir, None, &spec)?)?
        }
        RenderStyle::Belka => {
            let belka = magia_core::render::belka::belka_ir(&graph);
            ssr::render_via_ssr(&ssr::belka_request(&belka))?
        }
        RenderStyle::Yagami => anyhow::bail!("夜天の書式は Phase 6+ (未実装)"),
    };
    let svg = format!(
...
fn run_diff(
    ...
    if svg {
        ...
        // Phase 4.3 M5: 全経路 Vue SSR — overlay の意匠も Vue 1箇所が持つ。
        let (ir, marks) =
            magia_core::render::ir_export::diff_spell_ir(&before_graph, &after_graph, &spell_diff);
        let svg_text = ssr::render_via_ssr(&ssr::render_request(&ir, Some(&marks), &spec)?)?;
        let svg_text = format!(
            "<!-- generated by magia-cli v{} -->\n{svg_text}",
            env!("CARGO_PKG_VERSION")
        );
