...
/// Phase 4.3 M2: ミッドチルダ式の基本経路は Vue SSR (`magia-render`)。
/// スタンドアロン SVG (XML) としての契約を見る — `viewBox` の大文字保持、
/// hydration コメント・値なし data-v 属性 (XML 無効) が残っていないこと。
#[test]
fn render_default_path_is_ssr_with_valid_xml() {
    let output = magia()
        .arg("render")
        .arg(fixtures_dir().join("medium_render_doc.rs"))
        .arg("--fn")
        .arg("medium_render_doc")
        .output()
        .expect("CLI を起動できる");
    assert!(
        output.status.success(),
    ...
    let svg = String::from_utf8_lossy(&output.stdout);
    assert!(svg.contains(r#"viewBox=""#), "viewBox は大文字 (XML 規約)");
    assert!(!svg.contains("viewbox="), "小文字 viewbox が残っていない");
    assert!(
        !svg.contains("data-v-"),
        "scoped style の印は XML 無効なので落ちる"
    );
    // 数値ノイズ (sin/cos の 1e-15 等) は IR 側で2桁丸め済み。
    assert!(!svg.contains("e-1"), "指数表記の座標が漏れていない");
    // コメントはヘッダ1個 (generated by) のみ — hydration マーカーは残らない。
...
fn reducer_shape_prints_belka_hint() {
    ...
    // (Phase 4.3: SSR 経路は要素ごとの class — g.layer-* は出ない)。
    let svg = String::from_utf8_lossy(&output.stdout);
    assert!(svg.contains("main-ring") && !svg.contains("belka-pole"));
}
