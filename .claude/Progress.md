...
### Phase 4.0.6 前半 — シンボル凡例パネル (docs/plans/phase4.0.6-circle-affordances.md)

**スコープ**: 計画の前半 (凡例パネルのみ — レンダラ非依存で先行可、オーナー確定 2026-06-11)。後半 (入口・回転方向サイン / 補助陣ラベル) は 4.3 後。

**実装フェーズ**
- [x] 1. SymbolLegend.vue: 開閉式 (既定閉)。効果カテゴリ6色 / 図形8種 / 操作の強調 (ホバー=シアン・選択=金) / ベルカ三極
- [x] 2. 記号サンプルは実コンポーネント (RingCircle / OperationDot / SymbolMark / EdgeLine / GlyphDot) を再利用 — 意匠変更に凡例が自動追従 (selectable: false でクリック無効)
- [x] 3. HomeView 右カラム (パレットと関数一覧の間) に配置
- [x] 4. Playwright 1本 (既定閉 → 開で効果カテゴリ・図形・強調・ベルカの説明が見える)
- [x] 5. 素材作成 → オーナー判定待ち
- [x] 6. Stage 1 ゲート全通過 (vp check / vitest 30 / build / playwright 10 / cargo 17)。知見: 新規性低のため記録なし (実コンポーネント再利用は 4.0.7 設計の自然な帰結)

**品質検証 + 完了処理**
- [ ] 7. Stage 2 レビュー + コントリビューション検出 (ADDF 仕組み系のみ) + 指摘対応
- [ ] 8. 計画 memo、Feedback / TODO 更新、アーカイブ、コミット
