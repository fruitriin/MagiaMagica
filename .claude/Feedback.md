...
- **確認依頼 (Phase 4.0.5 M3 修正版, 2026-06-11)**: M3 判定対応済み — ペインを「魔法陣 (左・最大幅 1.6) | コード (中) | 関数一覧 (右端)」に並び替えました。**別スコープ要望4点は計画化済み**: 凡例リスト・入口/回転方向サイン・補助陣の関数名 → [phase4.0.6-circle-affordances.md](../docs/plans/phase4.0.6-circle-affordances.md) (優先度はオーナー確認待ち、凡例だけは 4.0.5 直後に先行可能)、TOC のピンフィルタ/ツリー表示 → phase4.1 に追補。並び OK なら **M4 (LayerPalette / DslEditor / TranscriptRegion の Vue 移植) に進みます**
