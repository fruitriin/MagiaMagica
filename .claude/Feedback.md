...
- **意匠判定依頼 (Phase 4.2, 2026-06-12)**: 近接度モデルの素材2点を送付済み — ①write_document (呼び出し関係7関数が中リング・他21が外リング) ②Shared::spell_json (同impl=内 / 呼び出し=中 / 他=外 の3層)。判定ポイント: リング間隔 (90/190/290)・チップ縮小/濃淡3段階 (0.55/0.45/0.35)・距離の直感 (同impl > 呼び出し > 同ファイル)。調整は `proximity.rs` の WEIGHT_* と `ir_export.rs` の RING_* 定数で対応
- **報告 (Phase 4.2 完了, 2026-06-12)**: 近接度モデルが完了しました (意匠判定待ち)。呼び出し関係が距離に効くようになり、ピン中心ビューが「同impl = 内 / 呼び出し相手 = 中 / その他 = 外」の3層になりました。関数間 call graph (`function_index_with_calls`) を新設 — 4.4 (呼び出しジャンプ) の入力としてそのまま使えます。**次は 4.3 (Vue SSR 一本化)** です
