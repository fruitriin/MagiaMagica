...
- **判定依頼 (Phase 4.0.5 M4, 2026-06-11)**: Phase 2.x 機能の Vue 移植が完了 — LayerPalette (式切替/レイヤー toggle + 透明度/全表示・全非表示/.magia DSL) を右カラム上部に、書き起こし region (visually-hidden) も移植済み。素材は既定/レイヤー操作中/ベルカの3状態 (M3 指示の新ペイン並びも反映)。判定ポイント: ①パレットの置き場 (fixed 右上 → 右カラム上部に変更した)、②Phase 2.x と機能等価か。URL は inline 版と完全互換 (?layers/?op/?style) で、リロード・直開きで全状態復元を E2E 確認済み。**判定が来たら M5 (rust-embed 統合 + 旧 inline HTML 削除) に進みます**。SVG 描画系アフォーダンスの 4.3 後実施も計画反映済みです
