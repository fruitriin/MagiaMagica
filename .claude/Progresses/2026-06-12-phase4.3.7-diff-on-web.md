...
- [x] 9. Stage 2: Critical/High なし。W1 (- 始まり rev のオプション注入防御) 修正、S1-S3 コメント化 (excerpt_maps 返り値順 / 初期化パスの同値スキップ / before 非キャッシュの理由)。S4 (git 設定の渡し方の形式差) は実害なし見送り。contribution: 候補なし。ゲート再通過 (vitest 41 / playwright 20)
- [x] 10. 計画 memo、knowhow (minimal-dev-server-pattern に live diff 節)、Feedback / TODO 更新、アーカイブ、コミット
