...
### シグネチャ円弧テキストのちぎれ修正 (オーナー報告 2026-06-12)

- [x] 原因特定: textPath は弧長 (π×半径) を超えるテキストを中央寄せの両端で切り捨てる
- [x] SignatureIr に arc_radius (Rust が半径を知っている)、spec §16 追補
- [x] fitToArc 純関数 (フォント縮小 0.1 刻み・下限 7px → 省略記号切り詰め)、vitest 4本
- [x] 実機確認 (lock_or_recover 全文収まり / 超長文は意図した末尾省略)
- [x] Stage 2 (近似係数 0.65 へ — 広字でも安全側)
