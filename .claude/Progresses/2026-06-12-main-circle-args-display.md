...
### 引数表示オプションをメイン魔法円にも (オーナー指示 2026-06-12)

- [x] ir::TypeInfo に fn_name / args / ret (serde default、構造体リテラル2箇所追従)
- [x] extract_type_info で部品抽出 (compact_tokens を signature.rs へ移動しチップと共有、明示的 -> () も None)
- [x] SignatureIr / IrSpell / Signature 型 / irToSchema に部品を素通し
- [x] SignatureArc: チェックボックス ON で組み立て、OFF は従来 text (SSR 静止画は無影響)
- [x] テスト (extract_type_info 部品 / e2e メイン円3モード)、spec §16 追補
- [x] Stage 1 全通過 (Rust 238 / vitest 41 / e2e 27) + Stage 2 (W1 () フィルタ / W2 spec 追補を対応)
