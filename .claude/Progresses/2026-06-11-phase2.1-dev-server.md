...
### Phase 2.1 — dev-server 最小実装 (docs/plans/phase2.1-dev-server.md)

**実装フェーズ**
- [x] 1. 依存選定: **tiny_http 0.12 + notify 8.2 + SSE** を採用（axum/tokio 一式を回避、spec §7 の「WebSocket または SSE」の範囲内）
- [x] 2. `magia-cli/src/serve.rs` — Shared 状態（svg / error / version / SSE clients）、初回レンダリング、HTTP 3ルート、リクエストごとスレッド、SseStream (Read アダプタで mpsc → チャンク転送)
- [x] 3. ファイル監視スレッド — 親ディレクトリ非再帰監視 + ファイル名照合（rename 保存対応）、120ms デバウンス、エラー時は直前 SVG 保持 + error 配信（会話を切らない）
- [x] 4. main.rs に `serve` サブコマンド追加（--port 既定 4747、0 で自動割当、起動 URL を flush 出力）
- [x] 5. 統合テスト3本: HTML/state/404 / ファイル変更で version 増加 + SVG 変化 / 構文エラー中の直前 SVG 保持 + 復帰 — 一発全通過
- [x] 6. ブラウザ実機確認 — preview サーバ経由で write_document の魔法陣表示をスクリーンショット確認（launch.json 新設）
- [x] 7. Stage 1 品質ゲート — 全通過（テスト121本、clippy 0）
- [x] 8. コーディング知見を /addf-knowhow で記録（minimal-dev-server-pattern.md 新設）

**品質検証フェーズ（Stage 2）**
- [x] 9. addf-code-review-agent でレビュー（Critical 0、High 2、Medium 4、Low 3）
- [x] 10. addf-contribution-agent でコントリビューション候補検出（分離違反なし、launch.json の ADDF scaffold テンプレ化提案 → Feedback に記録）
- [x] 11. レビュー指摘対応: H-1（poisoning 回復ロック）、H-2（SSE Sender の接続時掃除）、M-1/M-2（テスト堅牢化）→ 全ゲート再通過

**完了処理**
- [x] 12. 計画ファイルに実装結果メモ（tiny_http+SSE の実値判断含む）、Feedback / TODO 更新
- [x] 13. Progress.md アーカイブ → 新規作成、コミット
